        ...
        <v-card-text>
          <v-layout column>
            <v-layout v-for="(curPlan,i) in plans" :key="i">
              ...
              <v-btn @click.native.stop="$store.dispatch('planner/setCurPlan',{payload:i,$api})" :disabled="$store.getters['planner/currentPlan']===curPlan" color="primary">
                {{$store.getters['planner/currentPlan']===curPlan?'Is Current Plan':'Set as Current Plan'}}</v-btn>
              <v-tooltip bottom>
...
.in-planner>.v-card {
  /* add border radius and such to look like card*/
  background-image: repeating-linear-gradient(white 0px, white 24px, #a9b1d0 25px);
  border: 1px solid #ddd;
  border-radius: 100%;
  margin: 5px;
  box-shadow: 0 0 2px 1px rgba(76, 90, 95, 0.13);
}
</style>
